...
class HRNetW18SmallV1Config(HRNetConfig):
    """
    Configuration for HRNet-w18-small-v1
    """
    HRNET_NAME = "HRNet-w18-small-v1"
    # List of list; for each stage, the #channels per branch
    CHANNELS_PER_STAGE = [[32],
                          [16, 32],
                          [16, 32, 64],
                          [16, 32, 64, 128],
                          ]
    # List; for each stage, the #HR-modules (same across all branches)
    MODULES_PER_STAGE = [0, 1, 1, 1]
...
def hrnet_w18small_v1(input_tensor=None, input_shape=None, weights=None, **kwargs):
    config = HRNetW18SmallV1Config()
